...
5. As final stage, the author renames the `phep-proposal.md` file as `phep-ID.md` and includes the ID in the phep file header.
...
2. The reviewer does "the first pass" opening the discussion process using the comment system of the PRs (at this stage, any member of the community can give an opinion). 
3. If the reviewer considers the proposal needs to be accepted, this still counts as one (1) vote, and it needs to ask for the majority of the board to vote. 
4. When four (4) votes have been reached, the reviewer will assign an ID to the proposal.
5. When the author submits the "final stage", the reviewer merges the PR.
